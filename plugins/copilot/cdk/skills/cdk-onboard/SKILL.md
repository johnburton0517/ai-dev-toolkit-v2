...
> **CDK** — Core Development Kit skill. Invoke as `/cdk:cdk-onboard` in GitHub Copilot.
...
Inspect `.github/` (or Copilot plugin install location) for **uncommitted** cdk plugin files copied into the repo.

| Result | Action |
| --- | --- |
| PASS | No uncommitted plugin files, or user already committed |
| WARNING | Uncommitted plugin files found — ask user to **commit now** or **defer** (document choice) |
